...
url = 'https://api.github.com/repos/pruthvirajravi/VTM_CTC/actions/runs/32589283343/jobs'
req = urllib.request.Request(url, headers=headers)
with urllib.request.urlopen(req) as resp:
    jobs_data = json.loads(resp.read().decode())

failed_jobs = [j for j in jobs_data.get('jobs', []) if j['conclusion'] == 'failure']
print(f"Run #5: Total jobs = {len(jobs_data.get('jobs', []))}, Failed = {len(failed_jobs)}")
